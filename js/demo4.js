...
function Tunnel(cell, sickle) {
    window.cell = cell.children[0];
    window.sickle = sickle.children[0];
...
Tunnel.prototype.init = function () {
    ...
    this.scene.fog = new THREE.Fog(0xffffff, 0.05, 1.6); //@updated
...
Tunnel.prototype.updateCurve = function () {
    ...
    for (i = 0; i < this.tubeGeometry.vertices.length; i += 1) {
        ...
        vertice.x +=
            (vertice_o.x +
                this.splineMesh.geometry.vertices[index].x -
                vertice.x) /
            15;
        vertice.y +=
            (vertice_o.y +
                this.splineMesh.geometry.vertices[index].y -
                vertice.y) /
            15;
    }
...
Tunnel.prototype.render = function (time) {
    ...
    if (!this.mousedown) {
        if (time - this.prevTime > 20) {
            this.prevTime = time;
            var particle = new Particle(this.scene, true, true);
            this.particles.push(particle);
            this.particlesContainer.add(particle.mesh);
            if (!isMobile) {
                particle = new Particle(this.scene, true, true);
                this.particles.push(particle);
                this.particlesContainer.add(particle.mesh);
...
function Particle(scene, burst, isSickle) {
    var radius = Math.random() * 0.003 + 0.0003;
    var geom;
    if (isSickle) {
        geom = sickle.geometry;
    } else {
        geom = cell.geometry;
    }
    ...
    if (isSickle) {
        this.mesh.scale.x *= 0.6;
        this.mesh.scale.y *= 0.6;
        this.mesh.scale.z *= 0.6;
    }
}
...
function init() {
    ...
    loadingManager.onLoad = () => {
        window.tunnel = new Tunnel(red_cell_, sickle_cell_);
    };
    ...
    loader.load("img/demo4/red_blood_cell_sickle.obj", function (obj) {
        sickle_cell_ = obj;
    });
}
